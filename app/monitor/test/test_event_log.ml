...
(* Sample events carry wire ids; resolve them through the same default
   directory the harness uses, so rendered lines speak names (AAPL). *)
let lookup =
  Jsip_gateway.Symbol_directory.name
    (Jsip_gateway.Symbol_directory.of_symbols
       [ Harness.aapl; Harness.tsla; Harness.goog ])
;;
...
let%expect_test "fresh log has no events and no visible output" =
  let log = Event_log.create ~lookup in
...
let%expect_test "each event variant renders with its assigned color" =
  ...
  [%expect
    ...
    [cyan] FILL fill_id=1 AAPL $150.00 x100 aggressor=1|2(Alice) BUY resting=2|1(Bob)
...
let%expect_test "current_bbos tracks latest BBO per symbol in \
  ...
  =
  let aapl = Symbol_id.of_int_exn 0 in
  let tsla = Symbol_id.of_int_exn 1 in
